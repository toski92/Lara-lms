                ...
                <div class="lecture-overview-stats-wrap d-flex">
                    ...
                    <div class="lecture-overview-stats-item">
                        <ul class="generic-list-item">
                            <li><span>Skill level:</span>{{ $course->course->level }}</li>
                    ...
                    <div class="lecture-overview-stats-item">
                        <ul class="generic-list-item">
                            ...
                            <li><span>Duration</span>{{ $course->course->duration }}</li>
            ...
            <div class="question-overview-result-wrap">
                <div class="lecture-overview-item">
                    <div class="question-overview-result-header d-flex align-items-center justify-content-between">
                        <h3 class="fs-17 font-weight-semi-bold">{{ $questions->count() }} questions in this course</h3>
                ...
                <div class="lecture-overview-item mt-0">
                    <div class="question-list-item">
                        @php
                            $id = Auth::user()->id;
                            $question_asked = App\Models\Question::where('user_id',$id)->where('course_id',$course->course->id)->where('parent_id',null)->orderBy('id','asc')->get();
                        @endphp
                        @foreach ($question_asked as $que)
                        @php
                            $photo = $que->user->photo;
                            $user_img = empty($photo) ? asset('backend/assets/images/avatars/avatar-2.png') : asset("upload/$photo");
                        @endphp
                        <div class="media media-card border-bottom border-bottom-gray py-4 px-3">
                            <div class="media-img rounded-full flex-shrink-0 avatar-sm">
                                <img class="rounded-full" src="{{ $user_img }}" alt="User image">
                            </div>
                    ...
                    @php
                        $intructor_photo = $rep->instructor->photo;
                        $intructor_img = empty($intructor_photo) ? asset('backend/assets/images/avatars/avatar-2.png') : asset("upload/$intructor_photo");
                    @endphp
                    <div class="media media-card border-bottom border-bottom-gray py-4 px-3" style="background: #e6e6e6">
                        <div class="media-img rounded-full flex-shrink-0 avatar-sm">
                            <img class="rounded-full" src="{{ $intructor_img }}" alt="User image">
                        </div>
                        <div class="media-body">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="question-meta-content">
...
<div class="accordion generic-accordion generic--accordion" id="accordionCourseExample">
    ...
    @endphp
        ...
        <div class="card">
        <div class="card-header" id="headingOne{{ $topic->id }}">
            <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseOne{{ $topic->id }}" aria-expanded="true" aria-controls="collapseOne">
                <i class="la la-angle-down"></i>
                <i class="la la-angle-up"></i>
                <span class="fs-15">Topic {{ $topic_key+1 }}: {{ $topic->topic_title }}</span>
                <span class="course-duration">
                    <span>({{ count($lectures) }})</span>
                </span>
            </button>
        </div><!-- end card-header -->
        <div id="collapseOne{{ $topic->id }}" class="collapse " aria-labelledby="headingOne{{ $topic->id }}" data-parent="#accordionCourseExample">
            <div class="card-body p-0">
                <ul class="curriculum-sidebar-list">
                    @foreach ($lectures as $key => $lect)
                    <li class="course-item-link active">
                        <div class="course-item-content-wrap">
                            <div class="custom-control custom-checkbox">
